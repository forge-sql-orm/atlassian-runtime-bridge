...
public class AtlassianForgeUtils {
   ...
   *
   * <p>Atlassian GraphQL mutations such as {@code offlineUserAuthToken} expect {@code contextIds}
   * in ARI form; for Jira Cloud the site id is the last path segment of the product API base URL
   * (for example {@code https://api.atlassian.com/ex/jira/<cloudId>}).
   *
   * @param getApiBaseUrl Forge app {@code apiBaseUrl} (slash-separated URL string)
   * @return ARI of the form {@code ari:cloud:jira::site/<siteId>}, where {@code siteId} is the last
   *     path segment of {@code getApiBaseUrl}
   */
