...
/**
 * Connect-only delegation to {@link AtlassianHostRestClients}. Used by the {@code *ProductSelectAdapter}
 * beans when the request is <strong>not</strong> Forge-authenticated (no {@link com.atlassian.connect.spring.internal.auth.frc.ForgeAuthentication}).
 * <p>
 * Implements all three product adapter interfaces because Connect’s REST client factory exposes the
 * same entry points for Jira, Confluence, and generic routes.
 */
public class ConnectProductsAdapter implements ConfluenceProductAdapter, OtherProductAdapter, JiraProductAdapter {
