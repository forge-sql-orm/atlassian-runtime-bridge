 ...
 *
 * <p>If {@link SecurityContextHolder} holds a {@link ForgeAuthentication}, requests go through
 * {@link JiraProductForgeAdapter}; otherwise {@link ConnectProductsAdapter} (classic Connect {@link
 * AtlassianHostRestClients}) is used. Same interface for iframe and Forge Custom UI code paths.
 */
@Component
public class JiraProductSelectAdapter implements JiraProductAdapter {
  ...
  public RestTemplate authenticatedAsAddon(AtlassianHost host) {
    return isForge()
        ? jiraProductForgeAdapter.authenticatedAsAddon(host)
  ...
  public RestTemplate authenticatedAsCurrentUser() {
    return isForge()
        ? jiraProductForgeAdapter.authenticatedAsCurrentUser()
  ...
  public RestTemplate impersonation(AtlassianHostUser hostUser) {
    return isForge()
        ? jiraProductForgeAdapter.impersonation(hostUser)
