 ...
 * {@code com.github.vzakharchenko.runtime.bridge.forge.products.JiraProductSelectAdapter} bean that
 * <strong>automatically</strong> delegates to either Connect ({@code AtlassianHostRestClients}) or
 * Forge ({@code AtlassianForgeRestClients}) based on the current {@code SecurityContext}
 * (Forge requests use {@link com.atlassian.connect.spring.internal.auth.frc.ForgeAuthentication}).
 */
public interface JiraProductAdapter {
    ...
    /**
     * Calls Jira as a specific {@link AtlassianHostUser} (act-as / impersonation semantics).
     * On Forge this typically exchanges the app token for a user bearer via GraphQL impersonation.
     */
